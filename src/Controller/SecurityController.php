        ...
        if (Form::isSubmitted()) {
            ...
            if ($credentials && $password) {
                ...
                if (($user = $manager->findByUsernameOrEmail($credentials, $credentials))
                    ...
                    $this->addFlash('text-danger', 'Mauvais identifiants ou mot de passe, réessayez !');
                }
            } else {
            ...
            if ($nom && $email && $pass1) {
                if ($pass1 === $pass2) {
                    ...
                    if (!$manager->findByUsernameOrEmail($nom, $email)) {
                        ...
                        $this->addFlash('text-danger', 'Un utilisateur possède déjà cet email ou ce pseudo...');
                    }
                } else {
                    $this->addFlash('text-danger', 'pas les mm mots de passe');
                }
            } else {
                $this->addFlash('text-danger', 'passe pas les filtres');
            }
        }
